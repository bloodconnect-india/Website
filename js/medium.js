$(function () {
    ...
    var ourPost = {rss_url: 'https://medium.com/feed/@googledevs'};
            ...
            $.each(response.items,
                 function (k, item) {
                     ...
                     if(item.content.length > 300) {
                        ...
                        var title = item.title.substr(0,10)
                        ...
                        output += '<div class="blog-content m-4"><p class="mb-1"><a class="color-dark  bold" href="'+ item.link + '">' + title + '...</a></p>';
    ...
    var dkms = {rss_url: 'https://medium.com/feed/@DKMS_us'};
    $.get('https://api.rss2json.com/v1/api.json', dkms, 
    function (response) {
        if (response.status == 'ok') {
            var output = '';
            var count = 0;
            $.each(response.items,
                    ...
                    $("#skeleton-screen-dkms").hide()
                    // $content.html(output);
                    $('#dkms').slick('slickAdd',output);
                }});
